...
import { FaCamera, FaSpinner, FaEye, FaEyeSlash, FaHome, FaUser, FaSignInAlt, FaUserPlus, FaSignOutAlt, FaLock } from "react-icons/fa";
...
export default function Navbar() {
  ...
  const { data: user } = useQuery({
    ...
    queryFn: () =>
      axios
        .get("https://route-posts.routemisr.com/users/profile-data", { headers })
  ...
  const { mutate: uploadPhoto, isPending: isUploading } = useMutation({
    mutationFn: (formData) =>
      axios.put("https://route-posts.routemisr.com/users/upload-photo", formData, { headers }),
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 right-0 w-full bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-700 px-6 h-16 flex items-center justify-between transition-colors duration-300 z-50">
        {/* Brand */}
        <Link to="/" className="font-bold text-zinc-900 dark:text-white text-lg">
          SocialHub
        </Link>
        ...
        <div className="flex items-center gap-3">
              ...
              return (
                <Link
                  ...
                  className={`text-sm font-medium transition-colors ${
                    isActive ? "text-purple-600 dark:text-purple-400" : "text-zinc-600 dark:text-zinc-300 hover:text-zinc-900 dark:hover:text-white"
                  }`}
                >
                  {link.label}
                </Link>
            ...
            <div className="relative hidden sm:block" ref={dropdownRef}>
              ...
              {isOpen && (
                <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-xl shadow-lg z-50 overflow-hidden">
                  ...
                  <div className="px-4 py-3 border-b border-zinc-200 dark:border-zinc-700">
                    <div className="flex items-center gap-3">
                      <div className="relative flex-shrink-0">
                        ...
                        >
                          {isUploading ? <FaSpinner className="animate-spin" style={{ fontSize: "8px" }} /> : <FaCamera style={{ fontSize: "8px" }} />}
                        </button>
                        <input
                          ref={fileInputRef}
                          type="file"
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-purple-600 dark:text-purple-400 truncate">{userDataEmail}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  {showPasswordForm && (
                    <form onSubmit={handleSubmit(onPasswordSubmit)} className="px-4 pb-3 flex flex-col gap-2 border-t border-zinc-200 dark:border-zinc-700">
                      <div className="flex items-center bg-gray-50 dark:bg-zinc-800 rounded-lg px-3 py-2 mt-2">
                        ...
                        />
                        <button type="button" onClick={() => setShowPassword((p) => !p)} className="text-gray-400 text-xs">
                      ...
                      <div className="flex items-center bg-gray-50 dark:bg-zinc-800 rounded-lg px-3 py-2">
                        ...
                        />
                        <button type="button" onClick={() => setShowNewPassword((p) => !p)} className="text-gray-400 text-xs">
                          {showNewPassword ? <FaEyeSlash /> : <FaEye />}
                        </button>
                      </div>
                      {errors.newPassword && <p className="text-xs text-red-500">Min 8 characters.</p>}
                      ...
                      >
                        {isChanging ? <FaSpinner className="animate-spin" /> : "Save Password"}
                      </button>
                    </form>
                  )}
          ...
          >
            <span className={`block w-5 h-0.5 bg-zinc-700 dark:bg-zinc-300 transition-all duration-300 ${mobileMenuOpen ? "rotate-45 translate-y-2" : ""}`} />
            <span className={`block w-5 h-0.5 bg-zinc-700 dark:bg-zinc-300 transition-all duration-300 ${mobileMenuOpen ? "opacity-0" : ""}`} />
            <span className={`block w-5 h-0.5 bg-zinc-700 dark:bg-zinc-300 transition-all duration-300 ${mobileMenuOpen ? "-rotate-45 -translate-y-2" : ""}`} />
          </button>
        </div>
      </nav>
      ...
      {/* Mobile Menu */}
      <div className={`sm:hidden fixed top-16 left-0 right-0 bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-700 z-40 transition-all duration-300 ease-in-out ${mobileMenuOpen ? "max-h-screen opacity-100" : "max-h-0 opacity-0 overflow-hidden"}`}>
        ...
        {userLogin !== null && (
          <div className="px-4 py-4 border-b border-zinc-200 dark:border-zinc-700">
            <div className="flex items-center gap-3">
              <div className="relative flex-shrink-0">
                {user?.photo ? (
                  <img src={user.photo} alt="avatar" className="w-12 h-12 rounded-full object-cover border-2 border-purple-500" />
                ) : (
                  <div className="w-12 h-12 rounded-full bg-purple-100 dark:bg-purple-900 border-2 border-purple-500 flex items-center justify-center text-purple-600 dark:text-purple-300 font-medium">
                    {user?.name?.charAt(0) || "U"}
                  </div>
                )}
                <button
                  onClick={() => fileInputRef.current.click()}
                  disabled={isUploading}
                  className="absolute bottom-0 right-0 w-5 h-5 bg-purple-600 text-white rounded-full flex items-center justify-center shadow"
                >
                  {isUploading ? <FaSpinner className="animate-spin" style={{ fontSize: "8px" }} /> : <FaCamera style={{ fontSize: "8px" }} />}
                </button>
              </div>
              <div>
                ...
                <p className="text-xs text-purple-600 dark:text-purple-400">{userDataEmail}</p>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="flex flex-col py-2">
          ...
          {userLogin !== null && (
            <>
              ...
              {showPasswordForm && (
                <form onSubmit={handleSubmit(onPasswordSubmit)} className="px-6 pb-3 flex flex-col gap-2">
                  <div className="flex items-center bg-gray-50 dark:bg-zinc-800 rounded-lg px-3 py-2">
                    <input
                      type={showPassword ? "text" : "password"}
                      placeholder="Current password"
                  ...
                  <div className="flex items-center bg-gray-50 dark:bg-zinc-800 rounded-lg px-3 py-2">
                    <input
                      ...
                      {...register("newPassword", { required: true, minLength: 8 })}
                    />
                    <button type="button" onClick={() => setShowNewPassword((p) => !p)} className="text-gray-400">
                      {showNewPassword ? <FaEyeSlash /> : <FaEye />}
                    </button>
                  </div>
                  {errors.newPassword && <p className="text-xs text-red-500">Min 8 characters.</p>}
